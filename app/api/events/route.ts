...
export async function POST(request: NextRequest) {
  ...
  const { title, type = "Event", event_date = null, location = "", description = "", display_order = 0, published = 1 } = await request.json();
  if (!title) return NextResponse.json({ error: "Title is required" }, { status: 400 });

  const result = getDb()
    .prepare("INSERT INTO events (title, type, event_date, location, description, display_order, published) VALUES (?, ?, ?, ?, ?, ?, ?)")
    .run(title, type, event_date, location, description, display_order, published ? 1 : 0);
