...
export default function Home() {
  ...
  // Admin-managed content (falls back to hardcoded sections when empty)
  useEffect(() => {
    fetch("/api/videos").then((r) => r.json()).then((d) => setDbVideos((d.videos ?? []).slice(0, 3))).catch(() => null);
    fetch("/api/news").then((r) => r.json()).then((d) => setDbNews(d.posts ?? [])).catch(() => null);
    fetch("/api/events").then((r) => r.json()).then((d) => setDbEvents(d.events ?? [])).catch(() => null);
  }, []);
  ...
  return (
    <>
      <div className="opacity-100">
        ...
        >
          ...
          <div className="max-w-[1400px] mx-auto px-6 lg:px-10 relative z-10">
            ...
            <div className="space-y-4">
              ...
              ).map((event: { num: string; title: string; type: string; date: string; location: string }, i: number) => (
                <ScrollReveal key={i} variant="fade-up" delay={i * 60}>
                  <div key={`event-${i}`} className="flex items-center gap-6 p-5 rounded-xl border border-[#E2D9C8] hover:border-[#C9A84C]/40 hover:shadow-md transition-all bg-[#FDF8EF]/50 group">
                    <span className="text-[28px] font-bold text-[#C9A84C] min-w-[50px]">
                      {event.num || String(i + 1).padStart(2, "0")}
                    </span>
        ...
        >
          ...
          <div className="max-w-[1400px] mx-auto px-6 lg:px-10 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(dbNews.length > 0
                ? dbNews.slice(0, 6).map((n) => ({
                    title: n.title,
                    excerpt: n.excerpt,
                    date: new Date(n.published_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }),
                    views: n.views,
                    featured: !!n.featured,
                  }))
                : [
                ...
                <ScrollReveal key={`news-${i}`} variant="fade-up" delay={i * 60}>
        ...
        {/* ═══════════════════════════════════════════
             SECTION 9.5: Watch Boccia — YouTube videos from the admin
        ════════════════════════════════════════════ */}
        {dbVideos.length > 0 && (
          <section className="py-24 bg-[#0A2F1D] relative overflow-hidden" id="watch-boccia">
            <div className="absolute inset-0 bg-texture-dots-dark opacity-10 pointer-events-none" />
            <div className="gradient-orb gradient-orb-gold w-[500px] h-[500px] -top-40 right-1/4 pointer-events-none opacity-25 animate-float-slow gsap-parallax-orb" />
            <div className="max-w-[1400px] mx-auto px-6 lg:px-10 relative z-10">
              <ScrollReveal variant="fade-up">
                <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-14">
                  <div>
                    <span className="text-[#C9A84C] font-bold tracking-[0.25em] text-[12px] uppercase">
                      Boccia Jharkhand on YouTube
                    </span>
                    <h2 className="text-[36px] sm:text-[44px] font-bold text-white mt-2 gsap-parallax-heading">
                      Watch Boccia
                    </h2>
                  </div>
                  <Link
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {dbVideos.map((v, i) => (
                  <ScrollReveal key={v.id} variant="fade-up" delay={i * 90}>
                    <div className="group rounded-2xl overflow-hidden bg-white/5 border border-white/10 hover:border-[#C9A84C]/40 backdrop-blur-sm transition-all duration-300">
                      <YouTubeFacade youtubeId={v.youtube_id ?? ""} title={v.title} />
                      <div className="p-5">
                        <span className="px-2 py-0.5 rounded-full bg-[#C9A84C]/15 text-[#C9A84C] text-[10px] font-bold uppercase tracking-wider">
                          {v.category}
                        </span>
                        <h3 className="text-[15px] font-bold text-white mt-2 leading-snug group-hover:text-[#C9A84C] transition-colors">
                          {v.title}
                        </h3>
                      </div>
                    </div>
